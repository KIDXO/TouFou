...
- (void)commonInit
{
    ...
    _selectionIndicator.backgroundColor = self.selectionIndicatorColor ? self.selectionIndicatorColor : self.tintColor;
...
- (UIColor *)titleColorForState:(UIControlState)state
{
    ...
    if (!color) {
        switch (state) {
            case UIControlStateNormal:              return self.titleColor ? self.titleColor : [UIColor darkTextColor];
            case UIControlStateHighlighted:         return self.tintColor;
            case UIControlStateDisabled:            return self.countColor ? self.countColor : [UIColor lightTextColor];
...
- (void)setTitleColor:(UIColor *)color forState:(UIControlState)state
{
    ...
    for (UIButton *button in [self buttons]) {
        ...
        if (self.showsCount) {
            ...
            [attributedString addAttribute:NSFontAttributeName value:[UIFont fontWithName:self.font.fontName size:15.0f] range:[string rangeOfString:count]];
...
//                UIColor *bottomColor = self.inverseTitles ? color : [color colorWithAlphaComponent:0.5f];
                ...
                [attributedString addAttribute:NSForegroundColorAttributeName value:color range:NSMakeRange(0, topLength)];
                [attributedString addAttribute:NSForegroundColorAttributeName value:color range:NSMakeRange(topLength, bottomLength+1)];
            }
            else {
                [attributedString addAttribute:NSForegroundColorAttributeName value:color range:NSMakeRange(0, string.length)];
                
                if (state == UIControlStateSelected) {
                    self.selectionIndicator.backgroundColor = self.selectionIndicatorColor ? self.selectionIndicatorColor : self.tintColor;
                }
            }
        }
        else {
...
{
    _selectionIndicatorColor = color;
